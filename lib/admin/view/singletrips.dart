...
class SingleTrip extends StatefulWidget {
  ...
  const SingleTrip({Key? key, required this.vehicleId}) : super(key: key);
        ...
        children: [
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection('trips').where('vehicleid', isEqualTo: widget.vehicleId ).snapshots(),
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
        ...
        if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
          return const Text('No vehicle yet');
        }
